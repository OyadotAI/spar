  ...
  return (
    <div className="col" style={{ height: '100%' }}>
      ...
      <div className="fill" style={{ minHeight: 0 }}>
        <SplitPane storageKey="tests.split" initial={Math.max(360, window.innerWidth * 0.45)} min={240} minB={240}
          ...
          b={<div className="col" style={{ height: '100%' }}>
            {cases.length > 0 && <div style={{ maxHeight: '45%', overflow: 'auto', borderBottom: '1px solid var(--line)', fontSize: 12 }}>
              {cases.map((c) => <div key={c.name} className="row" style={{ padding: '2px 8px', alignItems: 'flex-start' }}>
                <span style={{ color: c.status === 'pass' ? 'var(--ok)' : c.status === 'skip' ? 'var(--warn)' : 'var(--err)', width: 14 }}>{c.status === 'pass' ? '✓' : c.status === 'skip' ? '↷' : '✗'}</span>
            ...
            {a?.result?.message && <div className="dim" style={{ padding: '4px 8px', fontSize: 12, whiteSpace: 'pre-wrap' }}>{a.result.message}</div>}
            <pre className="fill mono" style={{ margin: 0, padding: 8, overflow: 'auto', background: 'var(--bg-sunken)', fontSize: 11.5, whiteSpace: 'pre-wrap' }}>
