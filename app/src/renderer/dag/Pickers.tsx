...
export function CatalogPicker({ database, table, onChange }: { database: string; table: string; onChange: (db: string, table: string, columns?: { Name: string; Type: string }[]) => void }) {
  ...
  useEffect(() => { void api.get<Db[]>('/api/glue/catalog/databases', 'databases').then((r) => { if (r.ok) setDbs(r.value) }) }, [])
  useEffect(() => { setTables(null); if (database) void api.get<Tbl[]>(`/api/glue/catalog/tables?database=${encodeURIComponent(database)}`, 'tables').then((r) => { if (r.ok) setTables(r.value) }) }, [database])
  return (
    <div className="col" style={{ gap: 6 }}>
      ...
      {dbs && dbs.length === 0 && <span className="faint small">No databases in this region's Data Catalog.</span>}
    </div>
  )
}
