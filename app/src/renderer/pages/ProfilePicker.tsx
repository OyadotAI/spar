...
export function ProfilePicker() {
  ...
  const set = async (profile?: string, region?: string) => {
    setBusy(true)
    const r = await api.post('/api/profile', { profile, region }, 'the profile change')
    setBusy(false)
  ...
  return (
    <>
      <select disabled={busy} value={state?.profile ?? ''} onChange={(e) => { if (e.target.value === '__add') toggle('showSettings', true); else void set(e.target.value, undefined) }} title="AWS profile">
      ...
      <select disabled={busy} value={state?.region ?? ''} onChange={(e) => void set(undefined, e.target.value)} title="Region">
